    ...
    with gr.Row():
        with gr.Column(scale=1): 
            ...
            with gr.Tabs(selected=1) as tabs:
                ...
                with gr.Tab("Architecture Diagram", id=0):
                    gr.Markdown(BP_INFO_MARKDOWN)
                    gr.Image(value="frontend/static/diagram.png")

                with gr.Tab("Full End to End Flow", id=1):
                ...
                with gr.Tab("Agent Configurations", id=2):
                ...
                with gr.Tab("Podcast RAG", id=3):
    
                    with gr.Accordion("RAG data", open=False):
                        gr.Markdown("### Upload PDF Text and Dialogue JSON to Create a RAG Chatbot")
                    
                        with gr.Row():
                            pdf_text_file = gr.File(label="Upload PDF Text File", file_types=[".pdf"], file_count="multiple")
                            dialogue_json_file = gr.File(label="Upload Dialogue JSON File", file_types=[".json"], file_count="multiple")
